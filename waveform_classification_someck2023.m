...
% Returns:
%               pol         classification per channel: b-spike = 0, p-spike = 1, n-spike = -1
%               uType       classification per unit according to the main channel: 
%                               BIP = 3, Punit = 2, PYR/INT = 1
%               mch         main channel
...
function [ pol, uType, mch, ext, vB ] = waveform_classification( w, s, THs )
...
[ eidx, evals, etype ]       	= local_max( w2, 'ext' );
...
ext                             = NaN ( 1, n );
for i                           = 1 : n
    if pZ( i ) > THs( 1 ) && nZ( i ) < THs( 2 ) && pT( i ) < nT( i ) && ( vB( i ) > -0.6 && vB( i ) < 0.8 )
        if ~isnan ( vB( i ) )
            pol( i )            = 0;                                               % b-spike
            ext( i )            = pW(i)-nW(i);
        end
...
for i                           = 1 : n   
    w                           = s.mean{ i };
    sd                          = s.sd{ i };
    [pol0, uType0, mch0, ext0, vB0] = waveform_classification( w', sd' );
